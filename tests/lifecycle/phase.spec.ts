...
import {LifecyclePhaseId} from '../../src/lifecycle/phase/id';
import {LifecycleServer} from '../../src/lifecycle/server';
import {LifecycleServerDelegate} from '../../src/lifecycle/server/delegate';
...
describe('lifecyclePhase handler', () => {
	...
	describe('Client Phases', () => {
		for (const phase of CLIENT_PHASES) {
			it(`should return true for supported phase '${phase}'`, async () => {
				expect(await lifecyclePhase(sampleClient, phase)).toBe(true);
			});
		}
	});
});
